...
for file in glob.glob("data/*.json"):
    ...
    if not data['data']['site_list']:continue
    for site in data['data']['site_list']: # ['province_info', 'region_list', 'brand_list', 'site_list']
        res.append([site['provinceName'], site['cityName'], site['name'], site['address'], '/'.join(site['tels']), site['shopTime']])
df = pd.DataFrame(res)
df.to_excel("服务网点.xlsx")